...
CHAMPIONS_LEAGUE_SCHEDULE_REFRESH_WEEKDAY = 6  # Sunday in Europe/London
CHAMPIONS_LEAGUE_SCHEDULE_REFRESH_HOUR = 20
...
def competition_live_position_chart(conn, competition, matchday):
    rows = conn.execute(
        """SELECT s.captured_at, s.state_signature, s.cause_fixture_id, s.cause_label,
    ...
    if fixtures:
        ...
        if current_state != last_state:
            ...
            reconciled = {
                ...
                "label": captured.astimezone(UK).strftime("%H:%M"),
                "cause_fixture_id": _position_snapshot_cause(fixtures, captured)[0],
    ...
    for snapshot in snapshots:
        # Old CL rows have no causal fixture.  They cannot be accurately
        # relabelled after the fact, so omit them rather than claiming every
        # movement came from the same later scoreline.
        if (
            snapshot.get("cause_label") == "Live standings update"
            and not snapshot.get("cause_fixture_id")
        ):
            continue
        positions = tuple(sorted(
            (row["player_id"], row["position"])
...
def record_competition_live_position_snapshot(
    ...
    return _insert_competition_position_snapshot(
        conn, competition, matchday, now_utc().isoformat(), stored_signature, rows,
        cause, cause_fixture_id
    )


def _insert_competition_position_snapshot(
    conn, competition, matchday, captured_at, signature, rows, cause_label,
    cause_fixture_id=None,
):
    cursor = conn.execute(
        """INSERT OR IGNORE INTO competition_live_position_snapshots
           (competition, season, matchday, captured_at, state_signature,
            cause_label, cause_fixture_id)
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
...
def archive_completed_fixture_history(conn, competition=None):
    """Persist settled live fixtures for local form and H2H lookup.

    The active fixture table remains the preferred current-season source. This
    archive keeps those completed results available when future schedule
    refreshes replace the active fixture set.
    """
    filters = [
        "status = 'FINISHED'",
...
def premier_league_schedule_refresh_due():
    """Return the completed PL matchday due for its schedule refresh, if any."""
    conn = get_db()
    try:
        total = conn.execute("SELECT COUNT(*) FROM fixtures WHERE season = ? AND competition = 'premier_league'", (SEASON,)).fetchone()[0]
        if not total:
            return 'initial'
        row = conn.execute("""SELECT matchday, MAX(utc_date) AS final_kickoff FROM fixtures
            WHERE season = ? AND competition = 'premier_league' AND matchday IS NOT NULL
            GROUP BY matchday HAVING SUM(CASE WHEN status NOT IN ('FINISHED','CANCELLED') THEN 1 ELSE 0 END)=0
            ORDER BY matchday DESC LIMIT 1""", (SEASON,)).fetchone()
        if not row or not parse_utc(row['final_kickoff']):
            return False
        if now_utc() < parse_utc(row['final_kickoff']) + timedelta(hours=2):
            return False
        matchday = str(row['matchday'])
        return matchday if get_setting('pl_schedule_refresh_matchday') != matchday else None
    finally:
        conn.close()
    ...
    while True:
        try:
            if (
                ...
                imported = 0
                refreshed_matchday = premier_league_schedule_refresh_due()
                if refreshed_matchday:
                    imported = import_matches_from_api()
                    set_setting('pl_schedule_refresh_matchday', refreshed_matchday)
                    import_historical_results(seasons=[SEASON], include_championship=False)
...
@app.route("/champions-league/details")
def champions_league_details():
    if not logged_in(): return redirect("/")
    return render_template("champions_league_details.html")

@app.route("/head-to-head/details")
def head_to_head_details():
    if not logged_in(): return redirect("/")
...
def _live_football_test_clock(minute, injury_time):
    if minute is None:
        return ""
    return f"{minute}{'+' + str(injury_time) if injury_time else ''}'"


def _live_football_test_change_messages(previous, current):
    """Describe provider changes so latency can be checked from the test page."""
    if not previous:
        summary = f"Initial snapshot · {current['status_label']}"
        if current["score"] != [None, None]:
            summary += f" · score {current['score'][0] if current['score'][0] is not None else '–'}–{current['score'][1] if current['score'][1] is not None else '–'}"
        return [summary, *current["events"].values()]
    changes = []
    if previous.get("status") != current["status"]:
        changes.append(f"Status · {previous.get('status_label', 'unknown')} → {current['status_label']}")
    if previous.get("score") != current["score"]:
        changes.append(
            f"Score · {previous.get('score', [None, None])[0] if previous.get('score') else '–'}–{previous.get('score', [None, None])[1] if previous.get('score') else '–'} → {current['score'][0] if current['score'][0] is not None else '–'}–{current['score'][1] if current['score'][1] is not None else '–'}"
        )
    if previous.get("clock") != current["clock"] and current["clock"]:
        changes.append(f"Clock · {previous.get('clock') or '–'} → {current['clock']}")
    for event_key, label in current["events"].items():
        if event_key not in previous.get("events", {}):
...
def live_football_api_test():
    ...
    for match in provider_matches:
        ...
        event_labels = {}
        for goal in goals:
            event_key = f"goal|{goal['side']}|{goal['player']}|{goal['minute']}|{goal['penalty']}"
            event_labels[event_key] = f"Goal · {goal['player']} {goal['minute']}{' (Pen)' if goal['penalty'] else ''}"
        for card in cards:
